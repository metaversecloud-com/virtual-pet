...
// Release lock
await asset.updateDataObject({ lastUpdated: Date.now() }, { lock: { lockId, releaseLock: true } });
```
...
export class MyController extends SDKController implements MyInterface {
  ...
  async fetchData(): Promise<void | ResponseType> {
    try {
      const response: AxiosResponse = await this.topiaPublicApi().get(`/endpoint/${this.id}`, this.requestOptions);
